...
async def get_user_settings(from_user, key=None, edit_type=None, edit_mode=None):
    ...
    if key is None:
        ...
        capmsg = 'Not Exists' if (val:=user_dict.get('lcaption', '')) == '' else 'Exists'
...
@new_thread
async def user_settings(query, client, message):
    msg, button = await get_user_settings(message.from_user if sdirect else query.from_user, key, edit_type, edit_mode)
    user_id = query.from_user.id
    thumbpath = f"Thumbnails/{user_id}.jpg"
    if not ospath.exists(thumbpath):
        thumbpath = 'IMAGES'
